    ...
    def prepare_sequences(self, wait_trigger, start_with_event):
        ...
        if n_loops > 0:
            for params in self.parameters.items():
                ...
                self.write_in_database(params[0]+'_loop',
                                       np.linspace(loop_start,
                                                   loop_stop,
                                                   loop_points))

            loop_values = np.moveaxis(np.array(np.meshgrid(*loops)), 0, -1)
            loop_values = loop_values.reshape((-1, n_loops))
        else:
            loop_values = [1]
        ...
        for nn, loop_value in enumerate(loop_values):
            ...
            compilation = context.compile_loop(seq, for_file=True,
                                               factor=False)
                ...
                if ch_id in byteseq:
                    ...
                    for pos, waveform in enumerate(byteseq[ch_id]):
                        ...
                        if addr not in already_added:
                            seq_name_transfered = context.sequence_name +\
                                '_Ch{}'.format(ch_id) +\
                                '_' + str(pos)
