...
from atom.api import (Unicode, set_default, Tuple)

from exopy.tasks.api import (MultiInstrumentTask, validators)


class TuneIQMixerTask(MultiInstrumentTask):
    """ Task to calibrate IQ mixer

    """
    #: Switch to choose the AWG run mode: on or off

    AWG = Tuple(default=('', '', '', '')).tag(instr=True)
    SA = Tuple(default=('', '', '', '')).tag(instr=True)
